...
export default function ProgrammingJokesScreen() {
  ...
  useEffect(() => {
    const getJokes = async () => {
      try {
        ...
        const programmingJokes = allJokes.filter((joke) => joke.type === "programming");
        setJokes(programmingJokes);
        const max = programmingJokes.length;
        const randIndex = Math.floor(Math.random() * max);
        setTheJoke(programmingJokes[randIndex]);
        setLoading(false)
      } catch (error) {
